...
In this section, we define assumptions in collaborative learning settings and show that <span class="smallcaps">CoBo</span> converges to stationary points. The following assumptions regarding the local optimization objective \\(f_i\\) are commonly adopted in the literature `\citep{arjevani2023lower,koloskova2021unified}`{=latex}:
...
Due to time constraints, the proof of <a href="#theorem:main" data-reference-type="ref+Label" data-reference="theorem:main">1</a> is provided for a simplified scenario in which the inner problem employs the full gradient as per <a href="#eq:w" data-reference-type="eqref" data-reference="eq:w">[eq:w]</a>, and the minimization problem is solved exactly. Additionally, in the cross-device federated learning experiments involving 80 clients, each algorithm was executed only once due to the same time limitations. We will conduct repeated experiments to ensure the robustness of our findings in future iterations of this work.
