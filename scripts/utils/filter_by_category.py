...
def filter_limitation_only_entries(filtered_df, flawed_papers_source_dir):
    ...
    if filtered_out_count > 0:
        print(f"Warning: Filtered out {filtered_out_count} limitation-only entries (remaining: {len(filtered_df)} from {original_count})")
...
def get_available_paper_ids(papers_source_dir, use_submitted_version=False):
    """
    Get set of available paper IDs from a source directory.
    
    Args:
        papers_source_dir: Source directory containing papers
        use_submitted_version: If True, look for directories ending with 'v1' (default: False)
    
    Returns:
        Set of openreview_ids that are available in the source directory
    """
    ...
    for d in papers_source_dir.iterdir():
        if d.is_dir() and not d.name.startswith('.'):
            openreview_id = d.name.split('_')[0]
            
            if use_submitted_version:
                # For v1 version, match directories ending with 'v1'
                if d.name.endswith('v1'):
                    available_ids.add(openreview_id)
            else:
                # For latest version, match directories NOT ending with 'v1'
                if not d.name.endswith('v1'):
                    available_ids.add(openreview_id)
...
def copy_papers_for_category(
    ...
    Args:
        ...
        use_submitted_version: If True, use v1 submitted papers (directories ending with 'v1')
                               instead of latest versions (default: False)
    ...
    for d in papers_source_dir.iterdir():
        if d.is_dir() and not d.name.startswith('.'):
            ...
            if use_submitted_version:
                # For v1 version, match directories ending with 'v1'
                if d.name.endswith('v1'):
                    # Store with openreview_id as key, but keep the full directory name
                    source_paper_dirs[openreview_id] = d
            else:
                # For latest version, match directories NOT ending with 'v1'
                if not d.name.endswith('v1'):
                    source_paper_dirs[openreview_id] = d
    
    for openreview_id in tqdm(openreview_ids, desc=f"Copying papers to {subdirectory}", leave=False):
        # Find matching paper directory
        matching_dir = source_paper_dirs.get(openreview_id)
        ...
        # If using v1 version, remove 'v1' suffix from destination name
        source_dir = matching_dir
        if use_submitted_version and source_dir.name.endswith('v1'):
            dest_dir_name = source_dir.name[:-2]  # Remove 'v1' suffix
        else:
            dest_dir_name = source_dir.name
        
        dest_dir = output_dir / dest_dir_name
        ...
        try:
            ...
            print(f"Error copying {source_dir.name}: {e}")
            not_found_ids.append(openreview_id)
    
    if not_found_ids:
        print(f"Warning: {len(not_found_ids)} papers not found in source directory for {subdirectory}/")
...
def copy_flawed_papers_for_category(
    ...
    for openreview_id in tqdm(unique_openreview_ids, desc="Copying flawed papers", leave=False):
        ...
        try:
            ...
            if not csv_path.exists():
                ...
                if csv_files:
                    ...
                    print(f"Warning: No modifications_summary.csv found for {openreview_id}")
                    continue
            ...
            try:
                ...
                print(f"Error reading CSV for {openreview_id}: {e}")
                continue
                ...
                if flawed_papers_dir.exists() and flawed_papers_dir.is_dir():
                    ...
                    for flaw_id in filtered_modifications['flaw_id']:
                        ...
                        if md_file.exists():
                            ...
                            print(f"Warning: .md file not found for flaw {flaw_id} in {openreview_id}")
                else:
            ...
            print(f"Error processing {openreview_id}: {e}")
            not_found_ids.append(openreview_id)
    
    if not_found_ids:
        print(f"Warning: {len(not_found_ids)} papers not found in flawed papers source directory")
...
def filter_and_aggregate(
    ...
    submitted_papers_source_dir=None,
    ...
    Args:
        ...
        papers_source_dir: Source directory for latest/accepted papers (required if copy_papers=True and not using submitted version)
        submitted_papers_source_dir: Source directory for submitted (v1) papers (used when use_submitted_version=True)
        ...
        copy_csv_only: If True, only copy CSV files, not .md files (default: False)
        prune_appendix: If True, prune appendix sections from paper.md files (default: False)
        use_submitted_version: If True and submitted_papers_source_dir is provided, use v1 submitted papers instead of latest versions (default: False)
        no_duplicate_papers: If True, when sampling, ensure no duplicate papers (one flaw per paper) (default: False)
    """
    ...
    original_flaw_count = len(filtered_df)
    if n_samples is not None and n_samples > 0:
        if len(filtered_df) > n_samples:
            if no_duplicate_papers:
                # Sample unique papers first, then one flaw per paper
                unique_papers = filtered_df['openreview_id'].unique()
                if len(unique_papers) > n_samples:
                    # Sample papers using pandas for consistency
                    paper_df = pd.DataFrame({'openreview_id': unique_papers})
                    sampled_paper_df = paper_df.sample(n=n_samples, random_state=random_seed)
                    sampled_paper_ids = sampled_paper_df['openreview_id'].values
                else:
                    # Not enough unique papers, use all papers
                    sampled_paper_ids = unique_papers
                    print(f"Warning: Only {len(unique_papers)} unique papers available, but {n_samples} samples requested. Using all {len(unique_papers)} papers.")
                
                # For each sampled paper, randomly select one flaw
                sampled_rows = []
                for idx, paper_id in enumerate(sampled_paper_ids):
                    paper_flaws = filtered_df[filtered_df['openreview_id'] == paper_id]
                    if len(paper_flaws) > 1:
                        # Randomly select one flaw from this paper
                        # Use a different seed for each paper to ensure randomness
                        selected_flaw = paper_flaws.sample(n=1, random_state=random_seed + idx)
                        sampled_rows.append(selected_flaw)
                    else:
                        # Only one flaw for this paper
                        sampled_rows.append(paper_flaws)
                
                filtered_df = pd.concat(sampled_rows, ignore_index=True)
            else:
                # Original behavior: sample flaws (may have duplicate papers)
                filtered_df = filtered_df.sample(n=n_samples, random_state=random_seed)
    
    # --- Step 6.5: Filter to only include papers available in source directories ---
    if copy_papers and (papers_source_dir or submitted_papers_source_dir):
        available_paper_ids = set()
        
        # Get available papers from latest source directory
        if papers_source_dir:
            latest_available = get_available_paper_ids(papers_source_dir, use_submitted_version=False)
            available_paper_ids.update(latest_available)
        
        # Get available papers from v1 submitted source directory
        if use_submitted_version and submitted_papers_source_dir:
            v1_available = get_available_paper_ids(submitted_papers_source_dir, use_submitted_version=True)
            available_paper_ids.update(v1_available)
        
        # Filter DataFrame to only include papers that are available in at least one source
        if available_paper_ids:
            before_filter_count = len(filtered_df)
            filtered_df = filtered_df[filtered_df['openreview_id'].isin(available_paper_ids)].copy()
            filtered_out_count = before_filter_count - len(filtered_df)
            if filtered_out_count > 0:
                print(f"Warning: Filtered out {filtered_out_count} flaws for papers not available in source directories")
        else:
            print(f"Warning: No papers found in source directories, CSV will be empty")
    ...
    copied_count = 0
    copied_count_latest = 0
    copied_count_v1 = 0
    not_found_ids_latest = []
    not_found_ids_v1 = []
    flawed_copied_count = 0
    total_flaws_copied = 0
    flawed_not_found_ids = []
    unique_openreview_ids = set()
    
    # Determine output base directory
    if (copy_papers and (papers_source_dir or submitted_papers_source_dir)) or (copy_flawed_papers and flawed_papers_source_dir):
    ...
    # Copy latest papers if papers_source_dir is provided
    if copy_papers and papers_source_dir:
        copied_count_latest, not_found_ids_latest = copy_papers_for_category(
    ...
    # Copy v1 submitted papers if submitted_papers_source_dir is provided and use_submitted_version is set
    if copy_papers and use_submitted_version and submitted_papers_source_dir:
        copied_count_v1, not_found_ids_v1 = copy_papers_for_category(
            openreview_ids=unique_openreview_ids,
            papers_source_dir=submitted_papers_source_dir,
            output_base_dir=output_base_dir,
            category_id=category_id,
            venue_name=venue_name,
            subdirectory='v1',
            prune_appendix=prune_appendix,
            use_submitted_version=True
        )
    else:
        copied_count_v1 = 0
        not_found_ids_v1 = []
    
    # Update total copied count
    copied_count = copied_count_latest + copied_count_v1
    ...
    # Ensure unique_openreview_ids is initialized
    if not unique_openreview_ids:
        unique_openreview_ids = set(final_df['openreview_id'].dropna().unique())
    
    # Ensure output_base_dir is initialized for summary
    if 'output_base_dir' not in locals() or output_base_dir is None:
        output_base_dir = output_path.parent
    
    print(f"\n{'='*70}")
    print(f"Category {category_id} - Final Statistics")
    print(f"{'='*70}")
    print(f"\nDataset Summary:")
    print(f"  Total flaws: {len(final_df)}")
    if n_samples is not None and n_samples > 0 and original_flaw_count > n_samples:
        print(f"    (Sampled from {original_flaw_count} original flaws)")
    print(f"  Unique papers: {len(unique_openreview_ids)}")
    print(f"  Output CSV: {output_path}")
    
    if copy_papers and (papers_source_dir or (use_submitted_version and submitted_papers_source_dir)):
        print(f"\nPaper Copies:")
        if papers_source_dir:
            print(f"  Latest versions:")
            print(f"    Copied: {copied_count_latest} papers")
            if not_found_ids_latest:
                print(f"    Missing: {len(not_found_ids_latest)} papers")
            print(f"    Location: {output_base_dir}/{venue_name}/{category_id}/latest/")
        if use_submitted_version and submitted_papers_source_dir:
            print(f"  V1 submitted versions:")
            print(f"    Copied: {copied_count_v1} papers")
            if not_found_ids_v1:
                print(f"    Missing: {len(not_found_ids_v1)} papers")
            print(f"    Location: {output_base_dir}/{venue_name}/{category_id}/v1/")
    
    if copy_flawed_papers and flawed_papers_source_dir:
        print(f"\nFlawed Papers:")
        print(f"  Copied: {flawed_copied_count} papers with {total_flaws_copied} flaws")
        if flawed_not_found_ids:
            print(f"  Missing: {len(flawed_not_found_ids)} papers")
        if copy_csv_only:
            print(f"  Content: CSV files only")
        else:
            print(f"  Content: CSV files + flawed paper .md files")
        print(f"  Location: {output_base_dir}/{venue_name}/{category_id}/planted_error/")
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help='Source directory for latest/accepted papers (e.g., "data/original_papers/NeurIPS2024_pairs/latest"). Required if --copy_papers is used.'
    )
    parser.add_argument(
        '--submitted_papers_source_dir',
        type=str,
        default=None,
        help='Source directory for submitted (v1) papers (e.g., "data/original_papers/NeurIPS2024_pairs/v1"). Used when --use_submitted_version is set.'
    )
    parser.add_argument(
        '--copy_flawed_papers',
        action='store_true',
    ...
    parser.add_argument(
        '--use_submitted_version',
        action='store_true',
        help='When copying papers and --submitted_papers_source_dir is provided, use the submitted (v1) versions from that directory instead of latest versions. Papers must be in directories ending with "v1" (e.g., "0ZeONp33f0_2402_17805v1/").'
    )
    parser.add_argument(
    ...
    parser.add_argument(
        '--no_duplicate_papers',
        action='store_true',
        help='When sampling, ensure no duplicate papers (one flaw per paper). If enabled, samples unique papers first, then randomly selects one flaw from each paper.'
    )
    parser.add_argument(
        '--analyze_file_sizes',
        action='store_true',
    ...
    if args.copy_papers:
        ...
        if not args.papers_source_dir and not (args.use_submitted_version and args.submitted_papers_source_dir):
            if args.use_submitted_version:
                parser.error("--submitted_papers_source_dir is required when --copy_papers and --use_submitted_version are used (or provide --papers_source_dir to copy both latest and v1)")
            else:
                parser.error("--papers_source_dir is required when --copy_papers is used (or use --use_submitted_version with --submitted_papers_source_dir to copy only v1)")
        # If use_submitted_version is set, submitted_papers_source_dir must be provided
        if args.use_submitted_version and not args.submitted_papers_source_dir:
            parser.error("--submitted_papers_source_dir is required when --use_submitted_version is used")
    ...
    if args.category_id.lower() == 'all':
        ...
        for category_id in all_categories:
            ...
            filter_and_aggregate(
                ...
                submitted_papers_source_dir=args.submitted_papers_source_dir,
                ...
                use_submitted_version=args.use_submitted_version,
                no_duplicate_papers=args.no_duplicate_papers
            )
